...
public plugin_init() 
{
	...
	g_cvar_radiusdamage = register_cvar("cannon_radius_damage", "800.0")
	g_cvar_damage = register_cvar("cannon_damage", "574.0")
	
	register_clcmd("admin_get_dragoncannon", "get_dragoncannon", ADMIN_RCON)
...
public zp_user_infected_post(id) remove_dragoncannon(id)
public zp_user_humanized_post(id)
{
	if(zp_get_user_survivor(id)) get_dragoncannon(id)
	if(!zp_get_user_survivor(id)) remove_dragoncannon(id)
}
...
public check_radius_damage(id)
{
	...
	for(new i = 0; i < get_maxplayers(); i++)
	{
		...
		if(entity_range(id, i) >= get_pcvar_float(g_cvar_radiusdamage))
        	...
        	hook_ent2(i, MyOrigin, 570.0, 2) 

		new random_damage = get_pcvar_float(g_cvar_damage) * random_float(0.75, 1.25)
		ExecuteHamB(Ham_TakeDamage, i, 0, id, random_damage, DMG_BURN)
	}
}
