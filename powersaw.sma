...
#define V_MODEL "models/v_chainsaw.mdl"
#define P_MODEL "models/p_chainsaw.mdl"
#define W_MODEL "models/w_chainsaw.mdl"

#define DAMAGE_A 35	//Defailt: 35
#define DAMAGE_B 300	//Default: 240
#define CLIP 200
#define BPAMMO 600
...
#define ATTACK_RANGE 180.0

#define SLASH_TIME 1.0
#define SLASH_DELAY 0.1
#define SLASH_RANGE 200.0
#define SLASH_KNOCKPOWER 2800.0
...
{
	if(zp_get_user_survivor(id)) Get_PowerSaw(id)
	if(!zp_get_user_survivor(id)) Remove_PowerSaw(id)
}
	...
	if(get_gametime() - ATTACK_DELAY > g_Saw_AttackDelay[id])
	{
		...
		if(cs_get_weapon_ammo(Ent) > 0)
		{
			...
			if(is_user_alive(Target)) 
			{
				...
				//set_user_armor(Target, get_user_armor(Target) + DAMAGE_A/15)
	...
	for(new i = 0; i < get_maxplayers(); i++)
	{
		...
		|| get_distance_f(VicOrigin, Point[3]) <= Point_Dis)
		{
			...
			if(cs_get_user_team(id) != cs_get_user_team(i))
			{
				...
				//set_user_armor(i, get_user_armor(i) + DAMAGE_B/15)
			}
		}
	}	
