...
#define PLUGIN "Dual Nata Knives"
#define VERSION "1.0"
#define AUTHOR "JonVigo"

#define V_MODEL "models/v_nataknifedt.mdl"
#define P_MODEL "models/p_nataknifed.mdl"

#define CSW_NATAD CSW_KNIFE
#define weapon_natad "weapon_knife"
#define WEAPON_ANIMEXT "knife" //"skullaxe"

#define DRAW_TIME 1.0

#define SLASH_ROTATE_DAMAGE 0.0
#define SLASH_ROTATE_RADIUS 0.0
#define SLASH_ROTATE_POINT_DIS 0.0
#define SLASH_ROTATE_DELAY_TIME 0.0
#define SLASH_ROTATE_RESET_TIME 0.0

#define SLASH_AHEAD_DAMAGE 57.5
#define SLASH_AHEAD_RADIUS 75.0
#define SLASH_AHEAD_POINT_DIS 80.0
#define SLASH_AHEAD_DELAY_TIME 0.1
#define SLASH_AHEAD_RESET_TIME 1.0

#define STAB_DAMAGE 103.75
#define STAB_RADIUS 70.0
#define STAB_POINT_DIS 30.0
#define STAB_TIME 0.3
...
new const DualNata_Sound[8][] = 
{
	"weapons/nataknifed_draw.wav",
	"weapons/nataknifed_hit1.wav",
	"weapons/nataknifed_hit2.wav",
	"",
	"weapons/nataknifed_slash1.wav",
	"weapons/nataknifed_slash2.wav",
	"weapons/nataknifed_stab.wav",
	"weapons/nataknifed_hitwall.wav"
}
...
new g_Had_NataD[33], g_Slashing_Mode[33], g_Attack_Mode[33], g_Checking_Mode[33], g_Hit_Ing[33]
...
public plugin_init()
{
	...
	register_clcmd("admin_get_dualnata", "get_dualnata", ADMIN_KICK)
}
...
public plugin_precache()
{
	...
	for(new i = 0; i < sizeof(DualNata_Sound); i++)
		engfunc(EngFunc_PrecacheSound, DualNata_Sound[i])
}

public get_dualnata(id)
{
	if(!is_user_alive(id))
		return
	...
	if(get_user_weapon(id) == CSW_KNIFE)
	{
		...
		engclient_cmd(id, weapon_natad)
	}
}

public remove_dualnata(id)
{
	remove_task(id+TASK_SLASHING)
	remove_task(id+TASK_STABING)
...
public Event_CurWeapon(id)
{
	...
	if((read_data(2) == CSW_NATAD && g_Old_Weapon[id] != CSW_NATAD) && g_Had_NataD[id])
	{
		set_pev(id, pev_viewmodel2, V_MODEL)
		set_pev(id, pev_weaponmodel2, P_MODEL)
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if(/*get_user_weapon(id) != CSW_NATAD || */!g_Had_NataD[id])
		return FMRES_IGNORED
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	static ent; ent = fm_get_user_weapon_entity(id, CSW_NATAD)
	...
	if (CurButton & IN_ATTACK)
	{
		...
		if(!g_Slashing_Mode[id])
		{
			...
			set_weapons_timeidle(id, CSW_NATAD, SLASH_ROTATE_RESET_TIME)
			...
			set_weapons_timeidle(id, CSW_NATAD, SLASH_AHEAD_RESET_TIME)
		...
		set_pev(id, pev_framerate, 1.5)
		set_weapons_timeidle(id, CSW_NATAD, STAB_TIME + 0.1)
...
public Do_Slashing_Rotate(id)
{
	...
	{
		emit_sound(id, CHAN_WEAPON, DualNata_Sound[1], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DualNata_Sound[7], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, DualNata_Sound[4], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public Do_Slashing_Ahead(id)
{
	...
	{
		emit_sound(id, CHAN_WEAPON, DualNata_Sound[2], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DualNata_Sound[7], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, DualNata_Sound[5], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public Do_StabNow(id)
{
	...
	if(get_user_weapon(id) != CSW_NATAD)
	{
		set_weapons_timeidle(id, CSW_NATAD, 0.0)
		set_player_nextattack(id, 0.0)
	} else {
		set_weapons_timeidle(id, CSW_NATAD, STAB_RESET_TIME)
	...
	if(Check_Attack(id, ATTACK_STAB))
	{
		...
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DualNata_Sound[7], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, DualNata_Sound[6], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public fw_PlayerTraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], TraceResult, DamageBits) 
{
	if(!is_user_alive(Attacker))	
		return HAM_IGNORED
	if(!g_Had_NataD[Attacker] || !g_Checking_Mode[Attacker])
		return HAM_IGNORED
